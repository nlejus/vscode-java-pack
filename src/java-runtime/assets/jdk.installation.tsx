...
export class JdkInstallationPanel extends React.Component<JdkInstallationPanelProps, JdkInstallationPanelState> {
  constructor(props: JdkInstallationPanelProps) {
    super(props);
    this.state = {
      isLoading: _.isEmpty(props && props.data),
  ...
  static getDerivedStateFromProps(props: JdkInstallationPanelProps, state: JdkInstallationPanelState) {
    ...
    if (_.isEqual(state.data, props.data)) {
      return null;
    }
              ...
              <div className="form-row align-items-center">
                <div className="form-group col mb-1">
                  <h6 className="text-capitalize mb-2">
                    Download for&nbsp;
                  <span>{this.props.data && this.props.data.os}</span>&nbsp;
                  <span>{this.props.data && this.props.data.arch}</span>
                  </h6>
                </div>
              </div>
                ...
                <div className="form-group col-sm-12">
                  {
                    this.state.isLoading ?
                      ...
                      :
                      <a className="btn btn-primary btn-lg" href={this.props.data && this.props.data.downloadLink} role="button" title="Download JDK">
                        Download
                        <br />
                        <sub>{this.props.data && this.props.data.name}</sub><sub> | </sub><sub>{this.props.data && this.props.data.size}</sub>
                      </a>
                  }
                </div>
              </div>
